...
class RawSessionsStore:
    ...
    def get_record_type(self, record: dict) -> str:
        ...
        """
        print(f"Record type check: {record} (type: {type(record)})")
        keys = list(record.keys())[0:2]

        for record_type in RECORD_TYPE:
            if record_type in keys:
                return record_type
        logging.warning("Unknown record type. Skipping record.")
        return 'None'
    ...
    def validate_schema_record(self, record: dict, record_type: str) -> bool:
        ...
        try:
            if not record_type:
                logging.error("Record type is None. Cannot validate record.")
                raise ValueError("Record type is missing or invalid.")
                return False
                
            record_schema_path = os.path.join('data', record_type + '_schema.json')
            if not os.path.exists(record_schema_path):
                logging.error(f"Schema file not found: {record_schema_path}")
                raise FileNotFoundError(f"Missing schema file for record type '{record_type}': {record_schema_path}")
                return False
    ...
    def store_record(self, record: dict) -> bool:
        ...
        """
        self.check_connection()
        print(f"Record received: {record} (type: {type(record)})")
        if not isinstance(record, dict):
            logging.error(f"Invalid record type: {type(record)}")
            return False

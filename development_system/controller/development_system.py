...
class DevelopmentSystem:
    ...
    def run(self, productivity = False):
        ...
        while True:
            ...
            if message is True:
                print("[INFO] Start message processed.")
                continue  # Wait for dataset
            elif isinstance(message, dict):  # Ensure it's a valid dataset
                print("[INFO] Dataset received.")
                Dataset.set_data(message)
                self.update_stage("set_avg_hyp")
                break  # Exit the loop to proceed with the stages
            else:
                print("[ERROR] Unknown message received:", message)

        while True:
            # RECEIVE DATASET
            if self._configuration.stage == "waiting":
                dataset = MessageManager.get_instance().get_queue().get(block=True)
                print("_____________________________",dataset)
